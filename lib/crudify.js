...
module.exports = function(model, db){
	...
	crud.promiseList = function(queryMatch, querySlice, queryProject, querySort, queryPopulate){
		...
		Store.count(queryMatch, function(err, total) {
			if (err) {
				...
				var listOp = Store.find(queryMatch);
				if (querySort)
					listOp.sort(querySort);
				if (queryProject)
					listOp.project(queryProject);
				if(queryPopulate)
					listOp.populate(queryPopulate)
				if(queryProject)
					listOp.project(queryProject)
				if(querySlice.start)
					listOp.skip(querySlice.start-1)
				if(querySlice.limit && querySlice.limit > 0)
					listOp.limit(querySlice.limit)

				listOp.exec(function(err, results) {
					if (err) {
						console.log(err);
						deferred.reject({
							status: 500,
	...
	crud.list = function(prefs, config){
		return function(req, res){
			...
			if(config['hook']){
				if(config['hook']['hostParam'] && req.params[config['hook']['hostParam']] && config['hook']['linkField']){
					if(prefs.schema[config['hook']['linkField']] === "ObjectId" || prefs.schema[config['hook']['linkField']].type === "ObjectId"){
			...
			if(config['populate']){
				queryPopulate = config['populate'];
			}

			crud.promiseList(queryMatch,querySlice,queryProject, querySort, queryPopulate).then(function(result){
				res.jsonp(result);
			},function(error){
