...
var buildRoute = function(app, route, path, config, ctrl, collections, db) {
	...
	if (!config._meta.virtual) {
		...
		if (config._meta && config._meta.access) {
			...
			app.route(`/${path}`)
				.get(
					routesAuth.secureEndpoint('list', config._meta.access),
					methods.list(prefs, config._meta || {}))
				.post(
					routesAuth.secureEndpoint('create', config._meta.access),
	...
	for (var subroute in subroutes) {
		...
		*/
		...
		if (config._meta.virtual) {
			var tmpMeta = Object.assign({}, config._meta);
			tmpMeta.virtual = false;
			if (subconfig._meta) {
				subconfig._meta = Object.assign({}, subconfig._meta, tmpMeta);
			} else {
