...
const parseSchema = function(schemaStr, db = mongoose, returnJson = false, options = {}) {
	...
	Object.keys(schemaStr).forEach(field => {
		...
		if (schemaStr[field].constructor === Object) {
			if (schemaStr[field]['type'] && !schemaStr[field]['type']['type']) {
				...
				schemaValidator.validators
					.filter(validator => (typeof schemaStr[field][validator] !== 'undefined'))
					.forEach(validator => {
		...
		// if it is an array
			...
			if (schemaStr[field].length === 1) {
				...
				// array type definition [{type: String}]
					...
					schemaStr[field][0].constructor === Object
					&& schemaStr[field][0].type
					&& !schemaStr[field][0].type.type
				) {
